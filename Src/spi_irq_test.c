...
void SPI_App_Callback(void* param, uint8_t event) {
}
...
void spi_init(SPI_Handle_t* spi_handle) {
    ...
    spi_handle->cb_fn = SPI_App_Callback;
    spi_handle->cb_params = NULL;
	SPI_Init(spi_handle);
}
...
int main(void)
{
	...
	/* Enable SPI global interrupt in NVIC */
	SPI_IRQConfig(SPI2_IRQ_NUM, NVIC_IRQ_PRIORITY_12, 1);
...
void SPI2_IRQHandler(void) {
    SPI_IRQHandler(&spi_handle);
}

void EXTI0_IRQHandler() {
	GPIO_IRQHandler(GPIO_PIN_NO_0);
	SPI_Tx_NonBlocking(&spi_handle, (uint8_t*)tx_data, strlen(tx_data));
}
